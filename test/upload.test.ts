...
describe('worker - upload', () => {
	...
	it('upload: responds correctly for valid auth', async () => {
		const file = new File(['test'], 'test-upload.txt');
		const request = new IncomingRequest('https://cdn.4w3.dev/upload?filename=test-upload', {
			method: 'POST',
			body: file,
			headers: {
				'x-auth-key': 'test',
		...
		expect(results.image).toBe(`https://cdn.4w3.dev/file/${folder}/test-upload`);

		const urlParams = new URLSearchParams();
		urlParams.set('filename', `${folder}/test-upload`);
		urlParams.set('authkey', 'test');
		expect(results.deleteUrl).toBe(`https://cdn.4w3.dev/delete?${urlParams}`);
	});
});
