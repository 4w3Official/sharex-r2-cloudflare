...
describe('worker - get', () => {
	...
	it('file: responds correctly for valid auth', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/file/test.txt');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
		expect(response.status).toBe(200);
	...
	it('file: responds with cache correctly', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/file/test2.txt');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
		expect(response.status).toBe(200);
		...
		const request2 = new IncomingRequest('https://cdn.4w3.dev/file/test2.txt');
