...
describe('worker - unauthenticated', () => {
	it('responds with 404 for unknown route', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/404');
	...
	it('file: responds with 404 for missing file', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/file/missing');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
		expect(response.status).toBe(404);
	...
	it('file: responds with 404 for no filename', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/file/');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
		expect(response.status).toBe(404);
	...
	it('list: responds with 401 for missing auth', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/files/list');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
	...
	it('delete: responds with 401 for missing auth', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/delete?filename=test');
		const ctx = createExecutionContext();
		const response = await worker.fetch(request, env, ctx);
		await waitOnExecutionContext(ctx);
		expect(response.status).toBe(401);
	...
	it('upload: responds with 401 for missing auth', async () => {
		...
		const request = new IncomingRequest('https://cdn.4w3.dev/upload', {
			method: 'POST',
			body: file,
		});
		const ctx = createExecutionContext();
