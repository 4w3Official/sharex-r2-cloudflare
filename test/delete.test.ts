...
describe('worker - delete', () => {
	...
	it('delete: responds correctly for valid auth', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/delete?filename=test.txt', {
			headers: {
				'x-auth-key': 'test',
			},
		});
	...
	it('delete: after deleting, file is gone', async () => {
		const request = new IncomingRequest('https://cdn.4w3.dev/delete?filename=test2.txt', {
			headers: {
				'x-auth-key': 'test',
			},
		});
		...
		const request2 = new IncomingRequest('https://cdn.4w3.dev/test2.txt');
